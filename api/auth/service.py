    ...
    def register(data):
        try:
            ...
            if User.query.filter_by(email=email).first() is not None:
                return ErrResp("Email is used by another account.", "email_used", 403)
            ...
            if len(username) == 0 or username is None:
                return ErrResp("Username is required!", "invalid_username", 400)

            # Check if the username is being used
            elif User.query.filter_by(username=username).first() is not None:
                return ErrResp("Username is already taken!", "username_taken", 403)

            # Check if the username is equal to or between 4 and 15
            elif not 4 <= len(username) <= 15:
                return ErrResp("Username length is invalid!", "invalid_username", 400)
            ...
            if len(full_name) == 0 or full_name is None:
                ...
                if not full_name.replace(" ", "").isalpha():
                    return ErrResp("Name is not alphabetical.", "invalid_name", 400)

                # Check if the full name is equal to or between 2 and 50
                elif not 2 <= len(full_name) <= 50:
                    return ErrResp("Name length is invalid.", "invalid_name", 400)
